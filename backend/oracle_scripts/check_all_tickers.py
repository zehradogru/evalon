...
clean_expected_tickers = [x.replace('.IS', '') for x in TICKERS_RAW]
...
try:
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT COUNT(DISTINCT TICKER) FROM BIST_PRICES_1H")
        db_unique_tickers = cur.fetchone()[0]
        
        cur.execute("SELECT TICKER FROM BIST_PRICES_1H GROUP BY TICKER")
        db_tickers = [row[0] for row in cur.fetchall()]
        
        missing = set(clean_expected_tickers) - set(db_tickers)
        
        cur.execute("SELECT COUNT(*) FROM BIST_PRICES_1H")
        total_rows = cur.fetchone()[0]
        
        print("====== DURUM RAPORU ======")
        print(f"Hedeflenen Hisse Sayısı  : {len(clean_expected_tickers)}")
        print(f"DB'de Olan Hisse Sayısı  : {db_unique_tickers}")
        print(f"Kalan/Bekleyen Hisseler  : {len(missing)}")
        print(f"Toplam Çekilen Satır/Mum : {total_rows}")
        print("==========================")
