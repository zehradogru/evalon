...
def scrape_market(market: str, symbols: list, history: set, limit: int=100, sleep_sec: float=2.0, csv_path=None, csv_fields=None):
    ...
    # GNews nesnemizi olusturuyoruz (Gecmise donuk genis arama, son 1 yillik donem)
    google_news = GNews(language='tr' if market=='tr' else 'en', country='TR' if market=='tr' else 'US', period='1y', max_results=limit)
...
def main():
    ...
    for market, syms in markets.items():
        ...
        # limit=100 -> Her hisse için en güncelden geriye dogru max 50 haber
        # Anlik kayit icin csv_path gonderiliyor
        rows_count = scrape_market(market, syms, history, limit=50, sleep_sec=2.0, csv_path=csv_path, csv_fields=csv_fields)
